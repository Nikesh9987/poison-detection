...
st.markdown("""
<div class="hero-wrap">
  <div class="hero-badge">⬡ VishAI · Forensic Decision Support · v2.0</div>
  <h1 class="hero-title">Toxicology<br/>Intelligence System</h1>
  <p class="hero-sub">AI-assisted forensic toxicology decision support for probable poison identification in clinical and medicolegal contexts.</p>
  <div class="hero-disclaimer">
    ⚠️&nbsp; Preliminary output only. Laboratory confirmation is required before any medico-legal interpretation.
  </div>
</div>
""", unsafe_allow_html=True)
...
st.markdown('<p class="section-label">01 — Clinical Symptom Profile</p>', unsafe_allow_html=True)
...
with col_a:
    ...
    cns = st.multiselect(
        "CNS Symptoms",
        ["confusion", "coma", "delirium", "anxiety", "seizure", "headache"],
        key="cns"
    )
...
with col_b:
    ...
    resp = st.multiselect(
        ...
        ["dyspnoea", "tachypnoea", "respiratory failure", "pulmonary edema"],
        key="resp"
    )
...
with col_c:
    ...
    other = st.multiselect(
        ...
        ["fever", "cyanosis", "renal failure", "burning sensation"],
        key="other"
    )

    odour = st.selectbox(
        "Odour",
        ["Unknown", "Petroleum", "Garlic", "Sweet", "Bitter Almond", "None"],
        key="odour"
    )
...
with btn_col:
    predict_clicked = st.button("⬡  Run Toxicological Analysis", key="predict_btn")
...
if predict_clicked:
    ...
    with st.spinner("Analysing symptom profile…"):
        ...
        toxic_db = {
            "Datura": {
                "confidence": "HIGH",
                "severity":   "SEVERE",
                "confirmatory": "Urine alkaloid analysis\nGC-MS",
                "sample":       "Blood / Gastric lavage"
            },
            "Snakebite": {
                "confidence": "HIGH",
                "severity":   "CRITICAL",
                "confirmatory": "ELISA venom assay",
                "sample":       "Blood"
            },
            "Copper": {
                "confidence": "MODERATE",
                "severity":   "MODERATE",
                "confirmatory": "Serum copper estimation",
                "sample":       "Blood"
            },
            "Kerosene": {
                "confidence": "HIGH",
                "severity":   "SEVERE",
                "confirmatory": "Hydrocarbon toxicology",
                "sample":       "Blood / Gastric"
            }
        }
    ...
    <div class="footer-disclaimer">
      ...
      <div class="footer-disclaimer-text">
        <strong>Medico-legal Notice:</strong> This is a preliminary forensic decision-support output generated by an AI model.
        Laboratory confirmation is mandatory before any medico-legal interpretation. This tool does not replace
        expert forensic toxicological evaluation. All findings must be validated by a qualified professional.
